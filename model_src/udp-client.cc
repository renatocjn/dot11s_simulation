...
#include "ns3/random-variable.h" //ADD CARINA
#include <cstdlib>
#include <cstdio>
...
UdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpClient")
    ...
    */
     //ADD CARINA
    .AddAttribute ("Interval", "A RandomVariable used to pick the duration of the 'On' state.",
...
UdpClient::Send (void)
{
  ...
  if (m_sent < m_count)
    {
      //m_sendEvent = Simulator::Schedule (m_interval, &UdpClient::Send, this); //COMMENTED CARINA
      Time timeInterval = Seconds (m_interval.GetValue ()); //ADD CARINA
      m_sendEvent = Simulator::Schedule (timeInterval, &UdpClient::Send, this); //ADD CARINA
    }
}
