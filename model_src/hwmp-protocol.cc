        ...
        )
{
    ...
    HwmpTag tag;
    if (sourceIface == GetMeshPoint ()->GetIfIndex ()) //If this is the source
    {
        // packet from level 3
...
HwmpProtocol::SendProactivePreq ()
{
    ...
    for (HwmpProtocolMacMap::const_iterator i = m_interfaces.begin (); i != m_interfaces.end (); i++)
    {
		m_stats.initiatedProactivePreq++;
        i->second->SendPreq (preq);
    }
...
void HwmpProtocol::Statistics::Print (std::ostream & os) const
{
    os << "<Statistics "
		  ...
		  "initiatedProactivePreq=\"" << initiatedProactivePreq << "\" "
		  "forwardedPreq=\"" << forwardedPreq << "\"/>" << std::endl;
}
